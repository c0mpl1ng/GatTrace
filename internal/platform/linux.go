	...
	err := filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
		...
		if !info.IsDir() && time.Since(info.ModTime()) < 7*24*time.Hour {
			...
			fileInfo := core.FileInfo{
				...
				AccessTime: core.NormalizeTimestamp(time.Unix(int64(stat.Atim.Sec), int64(stat.Atim.Nsec))),
				ChangeTime: core.NormalizeTimestamp(time.Unix(int64(stat.Ctim.Sec), int64(stat.Ctim.Nsec))),
